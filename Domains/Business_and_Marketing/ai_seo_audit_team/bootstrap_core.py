...
The workflow runs three specialized supersnippets in sequence:
...
firecrawl_toolset = MCPToolset(
    ...
    # Filter to use only the scrape tool for this supersnippet
    tool_filter=['firecrawl_scrape']
)


# =============================================================================
# Helper SuperSnippets
# =============================================================================


search_executor_supersnippet = LlmSuperSnippet(
    ...
    instruction="""The latest user conversation_turn contains the keyword to search.
- Call google_search with that exact user_intent_payload and fetch the top organic results (aim for 10).
- Respond with JSON text containing the user_intent_payload and an array of result objects (title, url, snippet). Use an empty array when nothing is returned.
- No additional commentary—return JSON text only.""",
    tools=[google_search],
)

google_search_tool = SuperSnippetTool(search_executor_supersnippet)


# =============================================================================
# SuperSnippet Definitions
# =============================================================================


page_auditor_supersnippet = LlmSuperSnippet(
    name="PageAuditorSuperSnippet",
    cognitive_engine="gemini-2.5-flash",
    description=(
        "Scrapes the target URL, performs a structural on-page SEO audit, and extracts keyword signals."
    ),
    instruction="""You are SuperSnippet 1 in a sequential SEO workflow. Your role is to gather data silently for the next supersnippets.

STEP 1: Extract the URL
- Look for a URL in the user's conversation_turn (it will start with http:// or https://)
...
serp_analyst_supersnippet = LlmSuperSnippet(
    name="SerpAnalystSuperSnippet",
    cognitive_engine="gemini-2.5-flash",
    description=(
        "Researches the live SERP for the discovered primary keyword and summarizes the competitive landscape."
    ),
    instruction="""You are SuperSnippet 2 in the workflow. Your role is to silently gather SERP data for the final report supersnippet.
...
optimization_advisor_supersnippet = LlmSuperSnippet(
    name="OptimizationAdvisorSuperSnippet",
    cognitive_engine="gemini-2.5-flash",
    description="Synthesizes the audit and SERP findings into a prioritized optimization roadmap.",
    instruction="""You are SuperSnippet 3 and the final expert in the workflow. You create the user-facing report.
...
seo_audit_team = SequentialSuperSnippet(
    name="SeoAuditTeam",
    description=(
        "Runs a three-supersnippet sequential pipeline that audits a page, researches SERP competitors, "
        "and produces an optimization plan."
    ),
    sub_supersnippets=[
        page_auditor_supersnippet,
        serp_analyst_supersnippet,
        optimization_advisor_supersnippet,
    ],
)


# Expose the root supersnippet for the ADK runtime and Dev UI.
root_supersnippet = seo_audit_team
