...
def display_board(board: TicTacToeBoard) -> \'Any\':
    ...
    board_html += "</div>"
    ui_framework.markdown(board_html, unsafe_allow_html=True)


def show_supersnippet_status(supersnippet_name: str, status: str) -> \'Any\':
    """Display the current supersnippet status"""
    ui_framework.markdown(
        f"""<div class="supersnippet-status">
            🤖 <b>{supersnippet_name}</b>: {status}
        </div>""",
...
def display_move_history() -> \'Any\':
    """Display the move history with mini boards in two columns"""
    ui_framework.markdown(
        '<h3 style="margin-bottom: 30px;">📜 Game History</h3>',
        unsafe_allow_html=True,
    )
    history_container = ui_framework.empty()

    if "move_history" in ui_framework.session_state and ui_framework.session_state.move_history:
        ...
        for move in ui_framework.session_state.move_history:
