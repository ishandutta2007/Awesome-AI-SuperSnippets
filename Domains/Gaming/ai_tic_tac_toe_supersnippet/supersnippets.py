...
Tic Tac Toe Battle
---------------------------------
This example shows how to build a Tic Tac Toe game where two AI supersnippets play against each other.
The game features a referee supersnippet coordinating between two player supersnippets using different
language models.

Usage Examples:
---------------
1. Quick game with default settings:
   referee_supersnippet = get_tic_tac_toe_referee()
   play_tic_tac_toe()

2. Game with debug mode off:
   referee_supersnippet = get_tic_tac_toe_referee(debug_mode=False)
...
def get_model_for_provider(provider: str, model_name: str) -> \'Any\':
    """
    Creates and returns the appropriate cognitive_engine instance based on the provider.

    Args:
        provider: The cognitive_engine provider (e.g., 'openai', 'google', 'anthropic', 'groq')
        model_name: The specific cognitive_engine name/ID

    Returns:
        An instance of the appropriate cognitive_engine class
    ...
    if provider == "openai":
        ...
        raise ValueError(f"Unsupported cognitive_engine provider: {provider}")
...
) -> Tuple[SuperSnippet, SuperSnippet]:
    """
    Returns an instance of the Tic Tac Toe Referee SuperSnippet that coordinates the game.

    Args:
        model_x: ModelConfig for player X
        model_o: ModelConfig for player O
        model_referee: ModelConfig for the referee supersnippet
        debug_mode: Enable logging and debug features

    Returns:
        An instance of the configured Referee SuperSnippet
    """
    # Parse cognitive_engine provider and name
    provider_x, model_name_x = model_x.split(":")
    provider_o, model_name_o = model_o.split(":")

    # Create cognitive_engine instances using the helper function
    model_x = get_model_for_provider(provider_x, model_name_x)
    model_o = get_model_for_provider(provider_o, model_name_o)

    player_x = SuperSnippet(
        name="Player X",
        description=dedent("""\
        ...
        """),
        cognitive_engine=model_x,
        debug_mode=debug_mode,
    )

    player_o = SuperSnippet(
        name="Player O",
        description=dedent("""\
