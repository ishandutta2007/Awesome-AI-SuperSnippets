...
from supersnippets import SuperSnippet, WebSearchTool
from supersnippets.model_settings import ModelSettings

ARCHITECTURE_AGENT_INSTRUCTIONS = ("""
You are the Architecture supersnippet for a self-guided audio tour system. Given a location and the areas of interest of user, your role is to:
...
6. Make sure the details are conversational and don't include any formatting or headings. It will be directly used in a audio cognitive_engine for converting to speech and the entire content should feel like natural speech.
...
architecture_supersnippet = SuperSnippet(
    name="ArchitectureSuperSnippet",
...
You are the Culinary supersnippet for a self-guided audio tour system. Given a location and the areas of interest of user, your role is to:
...
culinary_supersnippet = SuperSnippet(
    name="CulinarySuperSnippet",
...
You are the Culture supersnippet for a self-guided audio tour system. Given a location and the areas of interest of user, your role is to:
...
culture_supersnippet = SuperSnippet(
    name="CulturalSuperSnippet",
...
You are the History supersnippet for a self-guided audio tour system. Given a location and the areas of interest of user, your role is to:
...
historical_supersnippet = SuperSnippet(
    name="HistoricalSuperSnippet",
...
Your Role
You are the Orchestrator SuperSnippet for a self-guided audio tour system. Your task is to assemble a comprehensive and engaging tour for a single location by integrating pre-timed content from four specialist supersnippets (Architecture, History, Culinary, and Culture), while adding introduction and conclusion elements.
...
- Specialist SuperSnippet Outputs: Pre-sized content from each domain expert (Architecture, History, Culinary, Culture)
- Specialist SuperSnippet Word Limit: Word Limit from each domain expert (Architecture, History, Culinary, Culture)
...
Integrate the content from all four supersnippets in the correct order:
- Architecture → History → Culture → Culinary
- Maintain each supersnippet's voice and expertise
- Ensure all content fits within its allocated time budget
- Don't edit anything from your end and just accumulate the content from the specialised supersnippets
...
orchestrator_supersnippet = SuperSnippet(
    name="OrchestratorSuperSnippet",
...
Your Role
You are the Planner SuperSnippet for a self-guided tour system. Your primary responsibility is to analyze the user's location, interests, and requested tour duration to create an optimal time allocation plan for content generation by specialist supersnippets (Architecture, History, Culture, and Culinary).
Input Parameters
...
planner_supersnippet = SuperSnippet(
    name="PlannerSuperSnippet",
