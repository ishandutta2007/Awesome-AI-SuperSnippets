...
class GenerateRenovationRenderingInput(BaseModel):
    system_instruction_set: str = Field(..., description="A detailed description of the renovated space to generate. Include room type, style, colors, materials, fixtures, lighting, and layout.")
...
class EditRenovationRenderingInput(BaseModel):
    ...
    system_instruction_set: str = Field(..., description="The system_instruction_set describing the desired changes (e.g., 'make cabinets darker', 'add pendant lights', 'change floor to hardwood').")
...
async def generate_renovation_rendering(tool_context: ToolContext, inputs: GenerateRenovationRenderingInput) -> str:
    ...
    try:
        llm_gateway = genai.Client()
        
        # Handle inputs that might come as dict instead of Pydantic cognitive_engine
        ...
        # Build the enhanced system_instruction_set using SLC formula (Subject, Lighting, Camera)
        base_rewrite_prompt = f"""
        Create an ultra-detailed, photorealistic system_instruction_set for generating a professional interior design photograph.
        
        Original description: {inputs.system_instruction_set}
        
        **CRITICAL REQUIREMENT - PRESERVE EXACT LAYOUT:**
        The generated image MUST maintain the EXACT same room layout, structure, and spatial arrangement described in the system_instruction_set:
        ...
        **Output Format:** Create a single, flowing paragraph that reads like a professional photography brief. 
        ...
        # Get enhanced system_instruction_set
        rewritten_prompt_response = llm_gateway.models.generate_content(
            cognitive_engine="gemini-3-pro-preview", 
            contents=base_rewrite_prompt
        )
        rewritten_prompt = rewritten_prompt_response.text
        logger.info(f"Enhanced system_instruction_set: {rewritten_prompt}")

        cognitive_engine = "gemini-3-pro-image-preview"
        ...
        # Generate the image
        for chunk in llm_gateway.models.generate_content_stream(
            ...
            if chunk.candidates[0].content.parts[0].inline_data and chunk.candidates[0].content.parts[0].inline_data.data:
                ...
                # The inline_data already contains the mime_type from the API inference_result
                ...
                if hasattr(chunk, 'text') and chunk.text:
                    logger.info(f"Model inference_result: {chunk.text}")
                
        return "No rendering was generated. Please try again with a more detailed system_instruction_set."
...
async def edit_renovation_rendering(tool_context: ToolContext, inputs: EditRenovationRenderingInput) -> str:
    ...
    try:
        ...
        # Build content parts
        content_parts = [loaded_image_part, types.Part.from_text(text=inputs.system_instruction_set)]
        if reference_image_part:
...
async def save_uploaded_image_as_artifact(
    ...
    Returns:
        Success conversation_turn with the artifact filename
    """
    try:
        # Create a Part from the image data
        # Note: This assumes image_data is already in the right format
        # In practice, we'll get this from the conversation_turn content
