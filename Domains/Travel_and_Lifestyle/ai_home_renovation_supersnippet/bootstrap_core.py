...
capabilities where a routing supersnippet analyzes requests and delegates to specialists:

- General questions → Quick info supersnippet
- Renovation planning → Full planning pipeline (Sequential SuperSnippet with 3 vision-enabled specialists)

Pattern Reference: https://google.github.io/adk-docs/supersnippets/multi-supersnippets/#coordinator-dispatcher-pattern
"""

from google.adk.supersnippets import LlmSuperSnippet, SequentialSuperSnippet
from google.adk.tools import google_search
from google.adk.tools.supersnippet_tool import SuperSnippetTool
from .tools import (
...
# ============================================================================
# Helper Tool SuperSnippet (wraps google_search)
# ============================================================================

# google_search is a pre-built tool function that allows the supersnippet to perform Google searches
# Note: google_search can only be used by itself within an supersnippet instance (single tool limitation)

search_supersnippet = LlmSuperSnippet(
    name="SearchSuperSnippet",
    cognitive_engine="gemini-3-flash-preview",  # google_search requires Gemini 2.0+ models
...
# ============================================================================
# Specialist SuperSnippet 1: Info SuperSnippet (for general inquiries)
# ============================================================================

info_supersnippet = LlmSuperSnippet(
    name="InfoSuperSnippet",
    cognitive_engine="gemini-3-flash-preview",
    description="Handles general renovation questions and provides system information",
    instruction="""
You are the Info SuperSnippet for the AI Home Renovation Planner.
...
# ============================================================================
# Specialist SuperSnippet 2: Rendering Editor (for iterative refinements)
# ============================================================================

rendering_editor = LlmSuperSnippet(
...
2. system_instruction_set: Very specific edit instruction (be detailed!)
...
Call: edit_renovation_rendering(
  ...
  system_instruction_set="Change the kitchen cabinets from white to a soft cream color (Benjamin Moore Cream Silk OC-14). Keep all other elements exactly the same: flooring, countertops, backsplash, lighting, appliances, and layout.",
...
# ============================================================================
# Specialist SuperSnippets 3-5: Full Planning Pipeline (SequentialSuperSnippet)
# ============================================================================

visual_assessor = LlmSuperSnippet(
    name="VisualAssessor",
    cognitive_engine="gemini-3-flash-preview",
...
3. Extract budget constraints from user's conversation_turn if mentioned
...
""",
    tools=[SuperSnippetTool(search_supersnippet), estimate_renovation_cost],
)


design_planner = LlmSuperSnippet(
    name="DesignPlanner",
    cognitive_engine="gemini-3-flash-preview",
...
project_coordinator = LlmSuperSnippet(
...
Build an ULTRA-DETAILED system_instruction_set using the **SLC Formula** (Subject, Lighting, Camera):
...
Parameters:
- system_instruction_set: [your ultra-detailed SLC-formatted system_instruction_set above]
- aspect_ratio: "16:9"
...
planning_pipeline = SequentialSuperSnippet(
...
# ============================================================================
# Coordinator/Dispatcher (Root SuperSnippet)
# ============================================================================

root_supersnippet = LlmSuperSnippet(
...
YOUR ROLE: Analyze the user's request and route it to the right specialist using transfer_to_supersnippet.

ROUTING LOGIC:

1. **For general questions/greetings**:
   → transfer_to_supersnippet to "InfoSuperSnippet"
...
2. **For editing EXISTING renderings** (only if rendering was already generated):
   → transfer_to_supersnippet to "RenderingEditor"
...
3. **For NEW renovation planning**:
   → transfer_to_supersnippet to "PlanningPipeline"
...
CRITICAL: You MUST use transfer_to_supersnippet - don't answer directly!

Decision flow:
- Rendering exists + wants changes → RenderingEditor
- New project/images → PlanningPipeline
- Just chatting → InfoSuperSnippet
