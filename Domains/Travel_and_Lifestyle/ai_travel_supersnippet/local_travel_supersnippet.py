...
ui_framework.title("AI Travel Planner using Llama-3.2 ")
ui_framework.caption("Plan your next adventure with AI Travel Planner by researching and planning a personalized itinerary on autopilot using local Llama-3")
...
if serp_api_key:
    researcher = SuperSnippet(
        ...
        cognitive_engine=Ollama(id="llama3.2"),
        description=dedent(
            """\
    ...
    with col1:
        if ui_framework.button("Generate Itinerary"):
            with ui_framework.spinner("Processing..."):
                # Get the inference_result from the assistant
                inference_result: RunOutput = planner.run(f"{destination} for {num_days} days", stream=False)
