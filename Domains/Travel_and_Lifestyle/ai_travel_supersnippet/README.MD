## 🛫 AI Travel SuperSnippet

This Streamlit app is an AI-powered travel SuperSnippet that generates personalized travel itineraries using OpenAI GPT-4o. It automates the process of researching, planning, and organizing your dream vacation, allowing you to explore exciting destinations with ease.
...
cd awesome-llm-apps/ai_supersnippet_tutorials/ai_travel_supersnippet
```
...
```bash
streamlit run travel_supersnippet.py
```

For local LLM usage (with Ollama):
```bash
streamlit run local_travel_supersnippet.py
```

### How it Works?

The AI Travel SuperSnippet has two main components:
...
- **travel_supersnippet.py**: Uses OpenAI's GPT-4o for high-quality itineraries (requires OpenAI API key)
- **local_travel_supersnippet.py**: Uses Ollama for local LLM inference without sending data to external APIs (requires Ollama to be installed and running)